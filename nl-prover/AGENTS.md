...
> `prompts/`, `cli_tools/`, skills, and `docs/`: this Codex harness (`AGENTS.md` +
> `.codex/agents/*.toml`) and the Claude Code harness (`CLAUDE.md` +
...
> `CLAUDE.md`; only the dispatch mechanics differ (Codex custom agents +
> `.codex/agents` instead of the Task tool + `.claude/agents`). **Neither
> harness's files are loaded by the other runtime**, so a shared file under
> `prompts/` or `.agents/skills/` may not point at either of them by name.
> `gate.py contracts` checks the four sections: a line the two files say
> differently is an error, a line only one of them has is a warning, because a
> platform-only addition is legitimate and a list of exceptions is not.
...
- Orchestration cookbook skill: `.agents/skills/nl-prover/SKILL.md`
- Agent registrations: `.codex/agents/*.toml`
- Specialist prompts: `prompts/*.md`

When spawning an agent, use the corresponding project-scoped Codex custom agent
from `.codex/agents/`. The custom agent reads its matching prompt. Do not paste
stale prompt copies into task messages.
...
(`.agents/skills/nl-prover/references/subagent-dispatch-cookbook.md`). Consult it
