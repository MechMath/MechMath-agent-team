#!/usr/bin/env python3
"""Cross-verify a proof using Gemini via OpenRouter when available."""
import argparse
import json
import logging
import os
...
from settings import GEMINI_API_KEY, OPENROUTER_API_KEY
...
def verify(proof_file: str, problem_file: str | None = None, lemma_file: str | None = None,
           model: str = "gemini-2.5-pro", temperature: float = 0.3) -> None:
    logger.info("gemini_verify.verify called: proof=%s model=%s", proof_file, model)
    ...
    if OPENROUTER_API_KEY:
        try:
            ...
            response = client.responses.create(
                model=normalize_model(model, "google"),
                input=prompt,
            ...
            logger.exception("gemini_verify.verify failed via OpenRouter: %s", e)
            print(json.dumps({"ok": False, "error": str(e)}))
            sys.exit(1)
    else:
        api_key = GEMINI_API_KEY
        if not api_key:
            print(json.dumps({"ok": False, "error": "OPENROUTER_API_KEY or GEMINI_API_KEY not set"}))
            sys.exit(1)
        ...
        try:
            ...
            logger.exception("gemini_verify.verify failed: %s", e)
            print(json.dumps({"ok": False, "error": str(e)}))
            sys.exit(1)
    ...
    logger.info("gemini_verify.verify succeeded: score=%s", score)
    print(json.dumps({"ok": True, "score": score, "analysis": text}, ensure_ascii=False))


def main(argv=None):
    parser = argparse.ArgumentParser(description="Cross-verify a proof using Gemini")
