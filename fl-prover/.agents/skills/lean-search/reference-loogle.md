...
```bash
python cli_tools/lean.py search loogle QUERY [-n NUM_RESULTS]
```
...
```bash
python cli_tools/lean.py search loogle "Real.sin"
python cli_tools/lean.py search loogle "_ * (_ ^ _)" -n 10
python cli_tools/lean.py search loogle "|- _ < _ → tsum _ < tsum _"
```
