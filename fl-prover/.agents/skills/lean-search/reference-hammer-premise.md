...
```bash
python cli_tools/lean.py search hammer GOAL [-n NUM_RESULTS]
```
...
```bash
python cli_tools/lean.py search hammer "⊢ ∀ n : ℕ, 0 ≤ n"
python cli_tools/lean.py search hammer "⊢ Real.exp x > 0" -n 20
```
