...
```bash
python cli_tools/external.py informal PROBLEM [OPTIONS]
```
...
```bash
python cli_tools/external.py informal "Prove that sqrt(2) is irrational" --backend gemini
python cli_tools/external.py informal "Prove the AM-GM inequality" --backend gpt --max-attempts 5
echo "Prove Fermat's little theorem" | python cli_tools/external.py informal - --backend gemini --model gemini-2.5-pro
```
